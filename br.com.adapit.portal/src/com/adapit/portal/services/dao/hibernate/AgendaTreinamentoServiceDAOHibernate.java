...
public class AgendaTreinamentoServiceDAOHibernate extends GenericDAOHibernate implements
	...
	public float updateIncremento(int idAgenda, float valor) throws Exception {
		...
		try {
			...
			float val = (Float) s.createQuery("select al.incremento from " + agendaTreinamento + " al where al.id=:id")
					.setParameter("id", idAgenda).uniqueResult();
			return val;
		} catch (Exception ex) {
			ex.printStackTrace();
	...
	public TurnoTreinamento getScheduledTrainingPeriod(int idAgenda) throws Exception {
		...
		try {
			s = LocalServicesUtility.getInstance().openSession();
			TurnoTreinamento regra = (TurnoTreinamento) s.createQuery(
	...
	public int getScheduledTrainingScheduleNumber(int idAgenda) throws Exception {
		...
		try {
			s = LocalServicesUtility.getInstance().openSession();
			int size = (Integer) s.createQuery("select size(t.agendas) from " + treinamentoEntityName
					+ " t where t.agendaTreinamento.id=" + idAgenda).uniqueResult();
			return size;
		} catch (Exception e) {
			e.printStackTrace();
			s.getTransaction().rollback();
	...
	@Override
	public EncerramentoCondicionado aprovarHomologacao(int idLote, EncerramentoCondicionado encCond,
			float valorAprovado) throws Exception {
		Session s = null;
		try {
			s = LocalServicesUtility.getInstance().openSession();
			s.beginTransaction();
			// s.update("status",lote);
			s.createQuery("update " + treinamentoEntityName + " l set l.status='"
					+ ScheduledTrainingStatus.Realizado.name() + "' where l.id=" + idLote).executeUpdate();
			encCond.setAprovado(true);
			encCond.setValorAprovado(valorAprovado);
			// s.update("aprovado,valorAprovado",encerramento.getEncerrCond());
			s.createQuery("update " + encerramentoCondicionadoName + " e set e.aprovado=true, e.valorAprovado="
					+ encCond.getValorAprovado() + " where e.id=" + encCond.getId()).executeUpdate();
			s.getTransaction().commit();
			return encCond;
		} catch (Exception e) {
			e.printStackTrace();
	...
	@Override
	public void confirmTrainingSchedule(AgendaTreinamento agenda, Treinamento treinamento, TurnoTreinamento turno)
			throws Exception {
		Session s = null;
		try {
			s = LocalServicesUtility.getInstance().openSession();
	...
	public void generateScheduleHistoryByScheduledTrainingId(int idLote) throws Exception {
		...
		try {
			...
			Treinamento lote = (Treinamento) s.load(Treinamento.class, idLote);
			if (lote == null)
				throw new ValidationException("Treinamento nao encontrado! Passe um identificador valido");
