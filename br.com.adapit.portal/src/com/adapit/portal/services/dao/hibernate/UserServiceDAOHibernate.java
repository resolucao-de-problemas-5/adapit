...
public class UserServiceDAOHibernate extends GenericDAOHibernate implements
	...
	/**
	 * retorna todos os usuario que alguma vez j� foram desativados
	 */
	...
	/**
	 * retorna todos os usuarios que sao do tipo de pessoa juridica
	 */
	...
	/**
	 * retorna todos os usuarios que sao do tipo de pessoa f�sica
	 */
	...
	/**
	 * Remove o usuario pelo identificador da tabela
	 */
	public Usuario deleteById(String id) throws Exception {
		try {
			...
			try {
				...
				Fisica f = (Fisica) s
						.createQuery("select pf from Fisica pf where pf.pessoa.id=" + user.getDadosPessoais().getId())
						.uniqueResult();
				Juridica j = (Juridica) s
						.createQuery("select pj from Juridica pj where pj.pessoa.id=" + user.getDadosPessoais().getId())
						.uniqueResult();
				...
				if (p instanceof Instrutor)
					...
					pa = (Participante) p;
				else if (p instanceof PessoaEmDivulgacao)
					co = (PessoaEmDivulgacao) p;
	...
	/**
	 * Remove o usuario pelo login de usuario
	 */
	...
	/**
	 * Remove o usuario pela senha de usuario
	 */
	...
	/**
	 * Retorna o usuario pelo identificador da tabela
	 */
	...
	/**
	 * Retorna o usuario pelo login e senha e estado de ativacao
	 */
	...
	/**
	 * Retorna o usuario pelo login e senha
	 */
	...
	public Integer countAccordingTo(String descricao, StringQueryKind descKind, UserDataFilterType un,
			UserCadastreType ur, UserSystemAccessFilterType byActive) throws Exception {
		try {
			List paramList = new ArrayList();
			StringBuffer hql = new StringBuffer();
			...
			if (descricao != null) {
				if (descKind == StringQueryKind.LIKE || descKind == StringQueryKind.BEGINS_WITH
						|| descKind == StringQueryKind.ENDS_WITH) {
					if (un == UserDataFilterType.PESSOA)
						hql.append(" lower(usuario.dadosPessoais.nome) like ? ");
					else
						hql.append(" lower(usuario.login) like ? ");
				} else if (descKind == StringQueryKind.EQUALS) {
					if (un == UserDataFilterType.PESSOA)
						hql.append(" lower(usuario.dadosPessoais.nome) = ? ");
					else
						hql.append(" lower(usuario.login) = ? ");
				}
			...
			if (!(byActive == UserSystemAccessFilterType.TODOS)) {
				...
				if (byActive == UserSystemAccessFilterType.ATIVOS)
					paramList.add(new Boolean(true));
				else
					paramList.add(new Boolean(false));
			}
	...
	public List listAccordingTo(String descricao, StringQueryKind descKind, UserDataFilterType un, UserCadastreType ur,
			UserSystemAccessFilterType byActive, int firstResult) throws Exception {
		int maxResults = FilterResultSize.userListMaxSize;
		try {
			List paramList = new ArrayList();
			StringBuffer hql = new StringBuffer();
			hql.append(
					"select usuario.login, usuario.userCadastreType, usuario.active, usuario.autenticado, usuario.dadosPessoais.id, usuario.dadosPessoais.nome, usuario.dadosPessoais.email, usuario.dadosPessoais.telefone from Usuario usuario ");
			if ((descricao != null || ur != null) || !(byActive == UserSystemAccessFilterType.TODOS))
				hql.append(" where ");
				...
				if (list != null && list.size() > 0) {
					for (Object[] objs : list) {
						try {
							...
							Pessoa p = null;
							if (user.getUserCadastreType() == UserCadastreType.Comitente)
								p = new PessoaEmDivulgacao();
							else if (user.getUserCadastreType() == UserCadastreType.Instrutor)
								p = new Instrutor();
							else if (user.getUserCadastreType() == UserCadastreType.Representante_legal)
								p = new RepresentanteLegal();
							else if (user.getUserCadastreType() == UserCadastreType.Administrador_do_sistema ||
									user.getUserCadastreType() == UserCadastreType.Funcionario)
								p = new Funcionario();
							else if (user.getUserCadastreType() == UserCadastreType.Cliente)
								p = new Participante();
							p.setId(idp);
							p.setNome(nome);
							p.setEmail(email);
							...
							System.out
									.println(" " + nome + " " + email + " " + telefone + " " + login + " " + r.name());
							try {

								Object[] tp = (Object[]) s.createQuery(
										"select tp.id, tp.razaoSocial, tp.cnpj, tp.inscricaoEstadual, tp.ramoAtividade from Juridica tp where tp.pessoa.id="
												+ p.getId())
										.uniqueResult();
								if (tp != null) {
									...
									tp = (Object[]) s.createQuery(
											"select tp.id, tp.sobrenome, tp.cpf, tp.rg from Fisica tp where tp.pessoa.id="
													+ p.getId())
											.uniqueResult();
									if (tp != null) {
										Fisica f = new Fisica();
	...
	public Usuario getUsuarioByIdCascadingProperties(java.io.Serializable id, boolean accessList,
			boolean deactivationReasons, boolean dadosPessoais) throws Exception {
		try {
			...
			if (accessList)
				org.hibernate.Hibernate.initialize(usuario.getAccessList());
			if (dadosPessoais)
				org.hibernate.Hibernate.initialize(usuario.getDadosPessoais());
			if (deactivationReasons)
				org.hibernate.Hibernate.initialize(usuario.getDeactivationReasonList());
			s.close();
			return usuario;
		} catch (Exception ex) {
			ex.printStackTrace();
	...
	public UsuarioDTO loggingUser(String login, String pass) throws Exception {
		...
		try {
			...
			if (u.getActive() && u.isAutenticado()) {
				...
				TipoPessoa tp = (TipoPessoa) s.createQuery("from Fisica f where f.pessoa.id=" + p.getId())
						.uniqueResult();
				if (tp == null)
					tp = (TipoPessoa) s.createQuery("from Juridica j where j.pessoa.id=" + p.getId()).uniqueResult();
				p.setTipoPessoa(tp);
				if (p.getTipoPessoa() instanceof Fisica)
					udto.setTipoPessoa(PersonType.Fisica);
				else
					udto.setTipoPessoa(PersonType.Juridica);
				udto.setParticipanteId(p.getId());
				udto.setUserRole(u.getUserCadastreType());
				udto.setWelcomeName(p.getNomeFormatado());

				try {
					UserLayoutPreferences pref = (UserLayoutPreferences) s
							.createQuery("select pref from UserLayoutPreferences pref where pref.login=:log")
				...
				try {
					List<AtributoPessoa> aList = s.getNamedQuery("personAttribute.listById")
	...
	public List<LoteReportDTO> getLotesParticipacao(String userid) throws Exception {
		...
		try {
			...
			if (p == null) {
				System.out.println("Nao encontrou o participante");
				return new ArrayList();
			} else if (p.getMeusTreinamentos() == null) {
				System.out.println("Nao existem lotes de prefer�ncia desse participante");
				return new ArrayList();
			}
			...
			while (it.hasNext()) {
				 ...
				 * lrd.setStatus(StatusAgenda.Em_execu��o_no_leilao_1);
				 * }
				 * else{
				 * if (sa != StatusAgenda.Cancelada &&
				 * sa != StatusAgenda.Em_execu��o_no_leilao_1 &&
				 * sa != StatusAgenda.Em_execu��o_no_leilao_2)
				 * lrd.setLeilaoIniciou(false);
				...
				/*
				 * String queryCount =
				 * "Select lance.valor From Lance lance, AgendaLote a where a.id="
				 * + l.getAgendaLote().getId()
				 * + " and a.lote.id="
				 * + l.getId()
				 * + " and a.status<>"
				 * + StatusAgenda.Encerrada_final_leilao_1.ordinal()
				 * + " and a.status<>"
				 * + StatusAgenda.Encerrada_final_leilao_2.ordinal()
			...
			while (it.hasNext()) {
				...
				if (userid != null) {
					String existsQuery = "Select u.dadosPessoais.id from Usuario u, Participante p, Lote l where u.login='"
							+ userid + "' and p.id = u.dadosPessoais.id and l.id =" + l.getId()
							+ " and l member of p.meusLotes";
					...
					if (obj instanceof Integer)
						lrd.setIdParticipante(((Integer) obj).intValue());
				} else {
					lrd.setLoteSelecionado(false);
				}
				lrd.setIdAgenda(l.getAgendaTreinamento().getId());
	...
	public Usuario deactiveUsuario(Usuario usuario, DeactivationReason deactivationReason) throws Exception {
		...
		try {
			...
			if (deactivationReason.getId() == 0)
				...
				s.merge(deactivationReason);
			/*
			 * String query =
			 * "update Usuario u set u.active=:active where u.login=:userlogin";
			 * s.createQuery(query)
			 */
	...
	public List<Usuario> listUsuariosAccordingTo(UserDataFilterType qKind, String value) {
		...
		String whereStr = "";
		if (qKind == UserDataFilterType.USUARIO)
			whereStr += " usuario.login ";
		else if (qKind == UserDataFilterType.PESSOA)
			whereStr += " usuario.dadosPessoais.nome ";
		else
			whereStr += " usuario.dadosPessoais.email ";
		try {
			List<Usuario> l = s.createQuery("from Usuario usuario where " + whereStr + " like '%" + value + "%'")
					.list();
			return l;
		} catch (Exception e1) {
			e1.printStackTrace();
	...
	public Usuario getByCpf(String cpf) {
		Session s = LocalServicesUtility.getInstance().openSession();
		try {
			Long pid = (Long) s.createQuery("select fis.pessoa.id from Fisica fis where fis.cpf='" + cpf + "'")
					.uniqueResult();
			if (pid != null) {
				Usuario user = (Usuario) s.createQuery("select u from Usuario u where u.dadosPessoais.id=" + pid)
						.uniqueResult();
				return user;
			}
		} catch (Exception e1) {
	...
	public Usuario getByCnpj(String cnpj) {
		Session s = LocalServicesUtility.getInstance().openSession();
		try {
			Long pid = (Long) s.createQuery("select jur.pessoa.id from Juridica jur where jur.cnpj='" + cnpj + "'")
					.uniqueResult();
			if (pid != null) {
				Usuario user = (Usuario) s.createQuery("select u from Usuario u where u.dadosPessoais.id=" + pid)
						.uniqueResult();
	...
	public Usuario getByInscricaoEstadual(String inscEst) {
		...
		try {
			Long pid = (Long) s
					.createQuery("select jur.pessoa.id from Juridica jur where jur.inscricaoEstadual='" + inscEst + "'")
					.uniqueResult();
			if (pid != null) {
				Usuario user = (Usuario) s.createQuery("select u from Usuario u where u.dadosPessoais.id=" + pid)
						.uniqueResult();
	...
	public Usuario getByRg(String rg) {
		Session s = LocalServicesUtility.getInstance().openSession();
		try {
			Long pid = (Long) s.createQuery("select fis.pessoa.id from Fisica fis where fis.rg='" + rg + "'")
					.uniqueResult();
			if (pid != null) {
				Usuario user = (Usuario) s.createQuery("select u from Usuario u where u.dadosPessoais.id=" + pid)
						.uniqueResult();
		...
		try {
			...
			try {
				...
				for (Usuario u : users) {
					 ...
					 * String
					 * delq="delete from Fisica fis where fis.pessoa.id="+u.getDadosPessoais().getId
					 * ();
					 * s.createQuery(delq).executeUpdate();
					 * }else{
					 * String
					 * delq="delete from Juridica jur where jur.pessoa.id="+u.getDadosPessoais().
					 * getId();
					 * s.createQuery(delq).executeUpdate();
					 * }
					 */
	...
	public List<Usuario> listUsuariosByPreference(
		...
		try {
			...
			s = LocalServicesUtility.getInstance().openSession();
			List<Usuario> list = (List<Usuario>) s
					.createQuery("select u from Usuario u where u.preferencia." + constraint + "=true")
					.list();
