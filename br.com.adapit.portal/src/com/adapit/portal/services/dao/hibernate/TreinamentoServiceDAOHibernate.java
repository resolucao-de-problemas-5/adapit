...
public class TreinamentoServiceDAOHibernate extends GenericDAOHibernate implements
	...
	/**
	 * Retorna todos os treinamentos que tiverem no codigo do treinamento o valor
	 * passado como
	 * argumento
	 */
	...
	/**
	 * Retorna todos os treinamentos em que o codigo termina com o valor passado
	 * como
	 * argumento
	 */
	@SuppressWarnings("unchecked")
	...
	/**
	 * Retorna todos os treinamentos em que o codigo inicia com o valor passado como
	 * argumento
	 */
	@SuppressWarnings("unchecked")
	...
	/**
	 * Retorna todos os treinamentos ou retirados ou nao retirados
	 */
	@SuppressWarnings("unchecked")
	@Deprecated
	...
	/**
	 * Retorna todos os treinamentos em que o status � igual ao valor passado como
	 * argumento
	 */
	@SuppressWarnings("unchecked")
	...
	/**
	 * Retorna todos os treinamentos de um comprador que possui no nome um valor
	 * igual
	...
	/**
	 * Retorna todos os treinamentos de um determinado pessoaEmDivulgacao que possua
	 * no nome o
	...
	/**
	 * Retorna todos os treinamentos de um determinado pessoaEmDivulgacao pelo
	 * identificador do
	 * pessoaEmDivulgacao
	 */
	@SuppressWarnings("unchecked")
	...
	/**
	 * apaga o treinamento que possui o codigo igual ao valor passado como argumento
	 */
	@SuppressWarnings("unchecked")
	...
	/**
	 * Apaga todos os treinamentos que forem de um pessoaEmDivulgacao. O
	 * pessoaEmDivulgacao � informado de
	...
	/**
	 * apaga todos os treinamentos que foram comprados por uma determinada pessoa.
	 * Esta
	...
	public AgendaTreinamento getScheduledTrainingScheduleIdByScheduledTrainingId(int treinamentoId) throws Exception {
		...
		try {
			int id = (Integer) s.createQuery("select treinamento.agendaTreinamento.id from "
					+ Treinamento.class.getSimpleName() + " treinamento where treinamento.id=" + treinamentoId + "")
					.uniqueResult();
			AgendaTreinamento agenda = new AgendaTreinamento();
			agenda.setId(id);
			// adicionei na transi��o
	...
	@SuppressWarnings("unchecked")
	@Override
	public Object filterScheduledTrainingBy(ScheduledTrainingFilterType tipoFiltro, int firstResultIndex,
		...
		if (tipoFiltro == null)
			throw new Exception("O tipo do filtro nao pode ser nulo");
		String query = null;
		if (tipoFiltro == ScheduledTrainingFilterType.Lote)
			...
			query = "select count(l) from " + entityName + " l";
		...
		if (tabSelectedIndex == 0) {
			...
			if (str == null || str.equals(""))
				...
				if (comparator1 != null)
					query += " and l.descricao like '%" + str + "%'";
				else {
			...
			if (str == null || str.equals(""))
				...
				if (comparator1 != null)
					query += " and l.processo.processo like '%" + str + "%'";
				else {
		...
		if (filtrarRetirados != null) {
			if (filtrarRetirados) {
				if (!whereused) {
					...
					query += " and l.retiradoDeposito=true";
			} else {
				if (!whereused)
					query += " where l.retiradoDeposito=false";
				else
					query += " and l.retiradoDeposito=false";
			}
			...
			if (filtrarArrematados) {
				...
				if (!whereused)
					query += " where l.arrematado=false";
				else
					query += " and l.arrematado=false";
			}
			...
			if (filtrarComPagamentoQuitado) {
				...
				if (!whereused)
					query += " where l.pagtoQuitado=false";
				else
					query += " and l.pagtoQuitado=false";
			}
		}
		...
		try {
			if (tipoFiltro == ScheduledTrainingFilterType.Lote) {
				List<Treinamento> treinamentos = s.createQuery(query).setMaxResults(maxResults)
						.setFirstResult(maxResults * firstResultIndex).list();
				return treinamentos;
			} else if (tipoFiltro == ScheduledTrainingFilterType.LoteCount) {
	...
	public Participante getAuthorByScheduledTrainingId(int idTreinamento) throws Exception {
		...
		try {
			...
			Long id = (Long) s.createQuery(
					"select treinamento.autor.id from " + entityName + " treinamento where treinamento.id="
							+ idTreinamento)
					.uniqueResult();
			if (id == null)
				return null;
	...
	public List<Treinamento> listAllScheduledTrainingByTurmaId(int idLeilao) throws Exception {
		...
		try {
			s = LocalServicesUtility.getInstance().openSession();
			List list = s
					.createQuery("select treinamento.id, treinamento.codigo from " + entityName
							+ " treinamento where treinamento.turma.id =" + idLeilao + " order by treinamento.codigo")
					.list();
	...
	public Treinamento updateScheduledTrainingProperties(Treinamento treinamento) throws Exception {
		...
		try {
			...
			Query query = s.createQuery("update " + entityName
					+ " l set l.resumo=:resumo, l.status=:status, l.custo=:custo, l.receita=:receita where l.id=:id");
	...
	public FormacaoTreinamento updateTrainingFormationProperties(FormacaoTreinamento treinamento) throws Exception {
		...
		try {
			...
			Query query = s.createQuery("update " + formacaoTreinamento
					+ " l set l.technology=:tec, l.nome=:nome, l.descricao=:desc, l.avaliacao=:avaliacao, l.cargaHorariaTotal=:ch where l.id=:id");
	...
	public Treinamento loadScheduledTrainingById(int idTreinamento, boolean cascadeAgendas) throws Exception {
		...
		try {
			Treinamento schedTrain = (Treinamento) s.createQuery("select treinamento from " + entityName
					+ " treinamento where treinamento.id =" + idTreinamento + " ").uniqueResult();

			/*
			 * if (schedTrain.getAgendaTreinamento()!= null)
			 * schedTrain.getAgendaTreinamento().getId();
			 */
			Integer agid = (Integer) s.createQuery(
					"select t.agendaTreinamento.id from " + entityName + " t where t.id =" + idTreinamento + " ")
					.uniqueResult();
			if (agid != null) {
			...
			Object[] objs = (Object[]) s
					.createQuery("select t.treinamento.id, t.treinamento.nome, t.treinamento.avaliacao from "
							+ entityName + " t where t.id =" + idTreinamento + " ")
					.uniqueResult();
	...
	public FormacaoTreinamento loadTrainingFormationByFormationId(int idTreinamento) throws Exception {
		...
		try {
			FormacaoTreinamento objLote = (FormacaoTreinamento) s.createQuery("select treinamento from "
					+ formacaoTreinamento + " treinamento where treinamento.id =" + idTreinamento + " ").uniqueResult();
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<ComercialSolutionItem> getComercialSolutionItensByScheduledTrainingId(int treinamentoId)
			throws Exception {
		List<ComercialSolutionItem> itens = new ArrayList<ComercialSolutionItem>();
		Session s = null;
		try {
			s = LocalServicesUtility.getInstance().openSession();
			String qItens = "select ip.id, ip.desconto, ip.valorAcertado from " + comercialSolutionItem
					+ " ip where ip.treinamento.id=" + treinamentoId;
	...
	public List<ComercialSolutionItem> getTrainingSolutionItensByFormationId(int formacaoId) throws Exception {
		...
		try {
			s = LocalServicesUtility.getInstance().openSession();
			String qItens = "select ip.id, ip.qtd, ip.valorTotal from " + comercialSolutionItem
					+ " ip where ip.formacao.id=" + formacaoId;
			List list = s.createQuery(qItens).list();
			if (list != null && list.size() > 0) {
				Iterator<Object[]> it = list.iterator();
				while (it.hasNext()) {
	...
	public List<Treinamento> createTrainemantosByFormacaoIdAndTurmaId(int fid, int tid) throws Exception {
		...
		try {
			...
			List<TrainingFormationItem> items = s
					.createQuery("select item from TrainingFormationItem item where item.trainingFormation.id=" + fid)
					.list();
			if (items != null) {
				for (TrainingFormationItem item : items) {
					...
					treinamento.setStatus(ScheduledTrainingStatus.Aguardando_confirmacao_da_agenda);
	...
	@SuppressWarnings("unchecked")
	@Override
	public void mergeNewTrainingSolutionOnScheduledTraining(TrainingSolution solution, Treinamento treinamento)
			throws Exception {
		Session s = LocalServicesUtility.getInstance().openSession();
		try {
			s.beginTransaction();
	...
	@SuppressWarnings("unchecked")
	@Override
	public ComercialSolutionItem mergeParticipanteOnScheduledTraining(Participante participante,
			Treinamento treinamento) throws Exception {
		Session s = LocalServicesUtility.getInstance().openSession();
		try {
			s.beginTransaction();
			...
			// s.refresh(prod);
			try {
				ComercialSolutionItem item = (ComercialSolutionItem) s
						.createQuery("select item from ComercialSolutionItem item where " +
								" item.treinamento.id=" + treinamento.getId() +
								" and item.inscrito.id=" + participante.getId())
						.uniqueResult();
				if (item != null)
					return item;
			} catch (Exception e) {
			...
			if (treinamento.getComercialSolutionItens() == null)
				treinamento.setComercialSolutionItens(new ArrayList());
			treinamento.getComercialSolutionItens().add(ip);
			if (participante.getItensTreinamentos() == null)
				participante.setItensTreinamentos(new ArrayList());
	...
	@SuppressWarnings("unchecked")
	@Override
	public TrainingFormationItem mergeNewTrainingSolutionOnTrainingFormation(TrainingSolution solution,
		...
		try {
			...
			if (treinamento.getTrainingFormationItens() == null)
				treinamento.setTrainingFormationItens(new ArrayList());
			treinamento.getTrainingFormationItens().add(ip);
			if (solution.getTrainingFormationItens() == null)
				solution.setTrainingFormationItens(new ArrayList());
	...
	/*
	 * public void removeTrainingSolutionFromScheduledTraining(boolean definitivo,
	 * int idItemProd) throws Exception{
	 * Session s = null;
	 * try {
	 * s = LocalServicesUtility.getInstance().openSession();
	 * 
	 * ComercialSolutionItem ip = (ComercialSolutionItem)
	 * s.load(ComercialSolutionItem.class,idItemProd);
	 * 
	 * s.beginTransaction();
	 * 
	 * if (definitivo){
	...
	public Object[] getScheduledTrainingItemPropertiesByItemId(int idItem) throws Exception {
		...
		try {
			s = LocalServicesUtility.getInstance().openSession();
			String queryprod = "select ip.inscrito.id, ip.inscrito.nome, ip.confirmada, ip.quitada, ip.desconto, ip.valorAcertado from "
					+ comercialSolutionItem + " ip where ip.id=" + idItem;
	...
	/*
	 * public Object[] getScheduledTrainingItemPropertiesByItemId(int idItem) throws
	 * Exception{
	 * Session s = null;
	 * try {
	 * s = LocalServicesUtility.getInstance().openSession();
	 * String queryprod =
	 * "select ip.comercialSolution.id, ip.comercialSolution.dataCriacao, ip.comercialSolution.nome, ip.comercialSolution.avaliacao, ip.comercialSolution.cargaHoraria from "
	 * +comercialSolutionItem+" ip where ip.id="+idItem;
	 * Object[] prods = (Object[]) s.createQuery(queryprod).uniqueResult();
	 * String catname = (String) s.
	 * createQuery("select prod.categoria.nome from ComercialSolution prod where prod.id="
	 * +prods[0]).uniqueResult();
	...
	public Object[] getTrainingFormationItemPropertiesByItemId(int idItem) throws Exception {
		...
		try {
			s = LocalServicesUtility.getInstance().openSession();
			String queryprod = "select ip.trainingSolution.id, ip.trainingSolution.dataCriacao, ip.trainingSolution.nome, ip.trainingSolution.avaliacao, ip.trainingSolution.cargaHoraria from "
					+ trainingFormationItem + " ip where ip.id=" + idItem;
			Object[] prods = (Object[]) s.createQuery(queryprod).uniqueResult();
			String catname = (String) s
					.createQuery(
							"select prod.categoria.nome from " + comercialSolution + " prod where prod.id=" + prods[0])
					.uniqueResult();
			Object objs[] = new Object[prods.length + 1];
			for (int i = 0; i < prods.length; i++) {
				objs[i] = prods[i];
	...
	public List<TrainingFormationItem> getTrainingFormationItensByFormationId(int formacaoId) throws Exception {
		...
		try {
			s = LocalServicesUtility.getInstance().openSession();
			String qItens = "select ip.id, ip.itemOrder from " + trainingFormationItem
					+ " ip where ip.trainingFormation.id=" + formacaoId + " order by ip.itemOrder ASC";
			List list = s.createQuery(qItens).list();
			if (list != null && list.size() > 0) {
				Iterator<Object[]> it = list.iterator();
				while (it.hasNext()) {
	...
	@Override
	public void createFormationWithTrainingSolutions(TrainingSolution prods[], FormacaoTreinamento treinamento)
			throws Exception {
		Session s = LocalServicesUtility.getInstance().openSession();
		try {
			...
			for (int i = 0; i < prods.length; i++) {
				...
				TrainingFormationItem ip = (TrainingFormationItem) s
						.createQuery(
								"select ip from " + trainingFormationItem
										+ " where ip.trainingFormation.id=:idf and ip.trainingSolution.id=:ids")
	...
	@Override
	@Deprecated
	public void createScheduledTrainingWithTrainingSolutions(TrainingSolution prods[], Treinamento treinamento)
			throws Exception {
		/*
		 * Session s = LocalServicesUtility.getInstance().openSession();
		 * try {
		 ...
		 * .createQuery(
		 * "select ip from "
		 * +comercialSolutionItem+" where ip.treinamento.id=:idt and ip.comercialSolution.id=:ids"
		 * )
	...
	/*
	 * @Override
	 * public void createScheduledTrainingWithTrainingSolutions(TrainingSolution
	 * prods[], Treinamento treinamento) throws Exception{
	 * Session s = LocalServicesUtility.getInstance().openSession();
	 * try {
	 * 
	 * for (int i = 0; i < prods.length; i++) {
