...
public class CadastroPessoaMultiActionController extends FormMultiActionController implements InitializingBean {
	...
	protected Object getCommand(HttpServletRequest request) {
		...
		if (usuario.getLogin() == null)
			usuario.setLogin(request.getParameter("id_usuario"));
			...
			try {
				...
				if (!usuario.isNewUser()) {
					...
					if (request.getSession(true).getAttribute("user") == null)
						throw new Exception("Nao � possivel atualizar um usuario nao logado!");
					s.update(endereco);
					s.update(pessoa);
					s.update(tipoPessoa);
	...
	protected BindException validate(HttpServletRequest request) throws Exception {
		...
		if (usuario == null || usuario.getLogin() == null)
			throw new Exception("Usuario e login nao podem ser nulos!");
		m = new HashMap();
		...
		if (errors == null || errors.getErrorCount() < 1) {
			 ...
			 * else if (!isValidEndereco(request)){
			 * //throw errors;
			 * ObjectError oe1 = new ObjectError("form", new String[]{"error"},
			 * null,"errors.default");
			 * errors.addError(oe1);
			 * //throw errors;
			 * }
			...
			System.out.println("Existem erros de formulario detectados na classe base");
		}
	...
	protected boolean canCadastreUsuario(HttpServletRequest request, Usuario usuario, BindException errors)
			throws Exception {
		if (usuario == null || usuario.getPassword() == null
				|| usuario.getPasswordConf() == null && usuario.isNewUser()) {
			ObjectError oe1 = new ObjectError("usuario", new String[] { "password", "passwordConf" },
					new Object[] { new Integer(6) }, "errors.minlength");
			ObjectError oe2 = new ObjectError("usuario", new String[] { "password", "passwordConf" },
		...
		if (usuario.isNewUser()) {
			if (usuario.getPassword().equals(usuario.getPasswordConf())) {
				...
				if (!(usuario.getPassword().length() < 6 || usuario.getPassword().length() > 8)) {
					if (usuario.isNewUser()) {
						...
						if (existe) {
							...
							ObjectError oe = new ObjectError("usuario", new String[] { "login" }, null,
									"errors.loginjustexist");
							errors.addError(oe);
							return false;
						} else {
							System.out.println("Usuario nao existe e pode ser cadastrado");
							return true;
						}
					} else
						return true;
				...
				if (!(usuario.getPassword().length() < 6 || usuario.getPassword().length() > 8)) {
					ObjectError oe1 = new ObjectError("usuario", new String[] { "password" },
	...
	@SuppressWarnings("unchecked")
	protected void bindTipoPessoa(HttpServletRequest request, java.util.Map m, ArrayList arr, Hashtable ht)
			throws Exception {
		...
		try {
			if (!CpfCnpj.isValid(request.getParameter("cpf"))) {
				/*
				 * ObjectError oe1 = new ObjectError("participante", new String[]{"cpf"},
				 * null,"errors.invalidcpf");
				 * errors.addError(oe1);
				 */
	...
	@SuppressWarnings("unchecked")
	protected void bindEndereco(HttpServletRequest request, java.util.Map m, ArrayList arr, Hashtable ht)
			throws Exception {
		...
		org.springframework.validation.BindException errors = new org.springframework.validation.BindException(endereco,
				"endereco");
	...
	@SuppressWarnings("unchecked")
	protected void bindPessoa(HttpServletRequest request, java.util.Map m, ArrayList arr, Hashtable ht)
			throws Exception {
		...
		org.springframework.validation.BindException errors = new org.springframework.validation.BindException(pessoa,
				"participante");
	...
	@SuppressWarnings("unchecked")
	protected void bindPreferencia(HttpServletRequest request, java.util.Map m, ArrayList arr, Hashtable ht)
			throws Exception {
