...
public class CadastrarInstrutorForm extends JPanel implements BindHandler {
	...
	protected JTabbedPane getTabbedPane() {
		...
		if (tabbedPane == null) {
			...
			tabbedPane.add(apresentacaoParticipante, "Apresenta��o");
	...
	protected AdminUserDataForm getAdminUserDataForm() {
		...
		if (adminUserDataForm == null) {
			...
			((JComboBox) adminUserDataForm.getRoleComboBox())
					.setSelectedItem(UserCadastreType.Instrutor.name().replace("_", " "));
		...
		if (deactivateButton == null) {
			...
			deactivateButton.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent arg0) {
					 ...
					 * try{
					 * //String query =
					 * "select d from DeactivationReason d where d.usuario.login=:userlogin";
	...
	public void editRegister() {
		...
		if (usuario == null)
			return;
		try {
			instrutor = (Instrutor) RemotePessoaService.getInstance()
					.initializeInstrutor(usuario.getDadosPessoais().getId());
		...
		if (user != null)
			try {
				...
				try {
					...
					if (cpfExists)
						getCadastrarLeiloeiroPanel().reportWarning("cpf");
					if (rgExists)
						getCadastrarLeiloeiroPanel().reportWarning("rg");
					if (emailExists)
						getCadastrarLeiloeiroPanel().reportWarning("email");
				...
				public void actionPerformed(ActionEvent arg0) {
					if (getParticipante().getId() != 0) {
						...
						JOptionPane.showMessageDialog(AdapitVirtualFrame.getInstance(),
								"Primeiro � preciso cadastrar a pessoa!", "Adicionar imagem",
		...
		public void actionPerformed(ActionEvent evt) {
			...
			if (rows != null) {
				try {
					...
					AdapitVirtualFrame.getInstance().showOperationSucess("Anexar Fotografia",
	 ...
	 * try {
	 * try {
	 * getParticipante().getLogotipo().setSmallImageBytes(RemoteImagemService.
	 * getInstance().getSmallImageBytesFromImage(getParticipante().getLogotipo().
	 * getId()));
	 * } catch (Exception e) {
	 * e.printStackTrace();
	 * }
	 ...
	 * ());
	 * getImgLabelImage().setIcon(getParticipante().getLogotipo().getSmallImageIcon(
	 * true));
		...
		if (getParticipante().getId() != 0) {
			try {
				...
				if (im != null) {
					getParticipante().setLogotipo(im);
					try {
						im.setSmallImageBytes(
								RemoteImagemService.getInstance().getSmallImageBytesFromImage(im.getId()));
