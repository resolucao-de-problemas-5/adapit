...
public class CadastrarParticipanteForm extends JPanel {
	...
	protected JPanel getPreferenciasPanel() {
		if (preferenciasPanel == null) {
			...
			JLabel acLabel = new JLabel("Lista das Preferencias");
			acLabel.setHorizontalTextPosition(JLabel.CENTER);
			...
			JButton jb = new JButton("Trocar Sele��o");
	...
	protected JPanel getCategoriasPanel() {
		if (categoriasPanel == null) {
			...
			JLabel acLabel = new JLabel("Lista das Categorias Preferidas do Usuario");
			acLabel.setHorizontalTextPosition(JLabel.CENTER);
	...
	/*
	 * @SuppressWarnings("unchecked")
	 * protected class PreferenciasList extends JXTable implements
	 ...
	 * super(values,new String[] {"Prefer�ncia","Sim/Nao"});
	 * }
	 * public Class getColumnClass(int columnIndex) {
	 * 
	 * return types[columnIndex];
	...
	protected JPanel getUserDataPanel() {
		...
		if (userDataPanel == null) {
			...
			// userDataPanel.setBorder(javax.swing.BorderFactory.createTitledBorder(javax.swing.BorderFactory.createTitledBorder("Dados
			// de Usuario")));
	...
	protected ParticipanteDadosPessoaFisicaPanel getComitenteDadosPessoaFisicaPanel() {
		if (cadastrarDadosPessoaFisicaPanel == null) {
			...
			cadastrarDadosPessoaFisicaPanel.getFisica().setProfissao("Empres�rio");
	 ...
	 * deleteButton = new JButton(messages.getMessage(
	 * "com.adapit.portal.ui.forms.manutencaousuario.CadastrarUsuarioAdmin.Apagar"))
	 * ;
	...
	private void updateImage() {
		if (getComitente().getLogotipo() != null)
			try {
				try {
					getComitente().getLogotipo().setSmallImageBytes(RemoteImagemService.getInstance()
							.getSmallImageBytesFromImage(getComitente().getLogotipo().getId()));
				} catch (Exception e) {
					e.printStackTrace();
				}
	...
	public void newRegister() {
		...
		try {
			...
			getPreferenciasList().pref = RemotePreferenciaService.getInstance().createPreferencia(true, usuario);
		} catch (Exception e) {
			e.printStackTrace();
		}
		if (personType == PersonType.Fisica) {
	...
	public void editRegister() {
		...
		try {
			tabbedPane.add(getGridCategoriasPanel(), "Preferencias e Categorias");
			tabbedPane.add(getGridLancesAcessosPanel(), "Acessos e Lances");
			// getPreferenciasList().createPreferencia(false);
			getPreferenciasList().pref = RemotePreferenciaService.getInstance().createPreferencia(false, usuario);
			getPreferenciasList().updateTable();
		...
		if (personType == PersonType.Fisica)
			getComitenteDadosPessoaFisicaPanel().editRegister(usuario.getDadosPessoais());
		else
			getComitenteDadosPessoaJuridicaPanel().editRegister(usuario.getDadosPessoais());
	...
	private class CadastrarAction extends AbstractAction {
		...
		protected void doAction(ActionEvent e) {
			try {
				...
				JOptionPane.showMessageDialog(AdapitVirtualFrame.getInstance(), "Problemas ao cadastrar usuario",
						"Cadastro de Usuario", JOptionPane.ERROR_MESSAGE);
				e1.printStackTrace();
			}
		}
	}
	...
	public void remover() {
		/*
		 * int resp = JOptionPane.showConfirmDialog(this, "Remover este usuario?");
		 ...
		 * LeilaoVirtualFrame.getInstance().
		 * displayErrorMsg("Nao foi possivel remover o usuario");
		 * LeilaoVirtualFrame.getInstance().showErrorDialog("Operacao: remover usuario",
		 * "O identificar do usuario nao foi informado");
		 * }else{
		 * LeilaoVirtualFrame.getInstance().beginStatusBar("Removendo usuario");
		 * try {
		 ...
		 * newRegister();
		 * LeilaoVirtualFrame.getInstance().
		 * setOperationMessage("Usuario removido com sucesso");
		 ...
		 * LeilaoVirtualFrame.getInstance().
		 * displayErrorMsg("Nao foi possivel remover o usuario");
		 * LeilaoVirtualFrame.getInstance().showErrorDialog("Erro de base de dados",
		 * "Problemas com acesso � base de dados do usuario");
		 * }
		 * LeilaoVirtualFrame.getInstance().endStatusBar("Removendo usuario");
		 * }
		 * 
		 * } catch (Exception e) {
		 * e.printStackTrace();
		...
		if (user != null)
			try {
				...
				try {
					...
					AdapitVirtualFrame.getInstance().setOperationMessage("Usuario cadastrado com sucesso");
					...
					if (cpfExists || rgExists || emailExists) {
						...
						AdapitVirtualFrame.getInstance()
								.setOperationMessage("Problemas ao cadastrar o usuario!" + '\n'
										+ "Verifique se j� nao existe algu�m " + '\n'
										+ "cadastrado com o mesmo cpf, rg ou email");
						AdapitVirtualFrame.getInstance().showErrorDialog("Dados duplicados o usuario",
								"Nao foi possivel cadastrar o usuario pois j� existem registros com os mesmos dados");
					} else {
						AdapitVirtualFrame.getInstance().endStatusBar("Salvando os dados do usuario");
	...
	public void cadastrar() throws Exception {
		...
		if (errorMsgs != null) {
			...
			try {
				AdapitVirtualFrame.getInstance().beginStatusBar("Verificando repeti��o de dados");
				try {
					validateUnique(p);
					AdapitVirtualFrame.getInstance().endStatusBar("Verificando repeti��o de dados");
	 ...
	 * LeilaoVirtualFrame.getInstance().
	 * beginStatusBar("Verificando a corretude dos dados");
	 * LeilaoVirtualFrame.getInstance().cleanErrorMsg();
	 ...
	 * 
	 * try {
	 * if (personType == PersonType.Fisica) p =
	 * getComitenteDadosPessoaFisicaPanel().cadastrePessoa();
	 * else p = getComitenteDadosPessoaJuridicaPanel().cadastrePessoa();
	 * } catch (Exception e) {
	 * e.printStackTrace();
	 * //JOptionPane.showMessageDialog(CadastrarUsuarioAdmin.this, e.getMessage(),
	 * "Formulario nao preenchido corretamente", JOptionPane.ERROR_MESSAGE);
	 * if (tipoComitenteComboBox != null && tipoComitenteComboBox.getSelectedItem()
	 * == null) getTabbedPane().setIconAt(2, warn);
	 * else{
	 * getTabbedPane().setIconAt(1, warn);
	 ...
	 * } catch (Exception e) {
	 * e.printStackTrace();
	 * //JOptionPane.showMessageDialog(CadastrarUsuarioAdmin.this, ex.getMessage(),
	 * "Formulario nao preenchido corretamente", JOptionPane.ERROR_MESSAGE);
	 ...
	 * if (errorMsgs != null){
	 * LeilaoVirtualFrame.getInstance().
	 * endStatusBar("Verificando a corretude dos dados");
	 * LeilaoVirtualFrame.getInstance().
	 * setOperationMessage("Cadastro suspenso ... verifique a corretude dos dados");
	 ...
	 * endStatusBar("Verificando a corretude dos dados");
	 * LeilaoVirtualFrame.getInstance().
	 * beginStatusBar("Salvando os dados do usuario");
	 * user.setPassword(Usuario.encrypt(user.getPassword()));
	 * user.setPasswordConf(user.getPassword());
	 * if (user!= null)try {
	 * 
	 ...
	 * s.beginTransaction();
	 * ender.setPais(Pais.Brasil);
	 * ((PessoaEmDivulgacao)p).setTipoComitente(TipoComitente.
	 * Individual_participante);
	 * ((PessoaEmDivulgacao)p).setLogotipo(getParticipante().getLogotipo());
	 * 
	 * if (!user.isNewUser()){
	 * s.update(ender);
	 ...
	 * 
	 * if (!usuario.getActive()){
	 * LeilaoVirtualFrame.getInstance().showOperationSucess("Envio de email",
	 * "Enviando email para o usuario autenticar o seu cadastro");
	 * RemoteAdapitAutenticateUserService.getInstance().sendAutenticateUserMsg(user)
	 * ;
	 * }
	 * LeilaoVirtualFrame.getInstance().endStatusBar("Salvando os dados do usuario"
	 * );
	 * LeilaoVirtualFrame.getInstance().
	 * setOperationMessage("Usuario cadastrado com sucesso");
	 * LeilaoVirtualFrame.getInstance().showOperationSucess();
	 * }catch(org.hibernate.exception.ConstraintViolationException ex){
	 * ex.printStackTrace();
	 * s.getTransaction().rollback();
	 ...
	 * +"Verifique se j� nao existe algu�m "+'\n'
	 * +"cadastrado com o mesmo cpf, rg ou email");
	 * LeilaoVirtualFrame.getInstance().showErrorDialog("Dados duplicados o usuario"
	 * ,
	 * "Nao foi possivel cadastrar o usuario pois j� existem registros com os mesmos dados"
	 * );
	 * }catch(org.hibernate.NonUniqueObjectException ex){
	 ...
	 * //usuario =
	 * LocalUserService.getInstance().getUserByLoginAndPassword(user.getLogin(),
	 * user.getPassword());
	 * 
	 ...
	 * setOperationMessage("Problemas ao cadastrar o usuario");
	 * LeilaoVirtualFrame.getInstance().
	 * showErrorDialog("Problemas para recuperar o usuario",
	 * "Nao foi possivel recuperar o usuario que foi cadastrado");
	 * }
	 * }
	 * }
	 */
