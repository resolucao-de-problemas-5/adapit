...
public class AgendaTreinamentoCadastreForm extends JPanel {
	...
	protected JLabel getNumeroLancesLabel() {
		if (numeroLancesLabel == null) {
			numeroLancesLabel = new JLabel("Numero de Lances:");
			...
			atualizarButton.addActionListener(new AbstractAction() {
				public void doAction(java.awt.event.ActionEvent e) {
					try {
						...
						e1.printStackTrace();
						AdapitVirtualFrame.getInstance().showErrorDialog("Problemas para cadastrar/atualizar a agenda",
								"Nao foi possivel cadastrar ou atualizar a agenda");
					}
				}
			});
		}
	...
	private AgendaTreinamento validateAgendaTreinamento() throws Exception {
		...
		try {
			...
			agenda.setStatus(
					StatusAgenda.valueOf(((String) statusAgendaComboBox.getSelectedItem()).replaceAll(" ", "_")));
			...
			if (agenda.getInicioPrevisto() == null) {
				reportInvalidField("inicioPrevisto", "nao pode ser vazia");
				invalid = true;
			}
			...
			if (agenda.getTerminoPrevisto() == null) {
				reportInvalidField("terminoPrevisto", "nao pode ser vazia");
				invalid = true;
			}
	...
	private AgendaTreinamento cadastreAgendaTreinamento() throws Exception {
		...
		try {
			a = RemoteAgendaTreinamentoService.getInstance().saveOrUpdateMergeScheduledTrainingId(a,
					treinamento.getId());
	...
	protected JButton getConfirmarButton() {
		...
		if (agendarButton == null) {
			agendarButton = new JButton("Confirmar Agenda");// messages.getMessage("com.adapit.portal.ui.forms.baseleilao.AgendaTreinamentoCadastreForm.ConfirmaroIniciodoLeilao"));
			...
			agendarButton.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					try {
						...
						TurnoTreinamento chamada = TurnoTreinamento
								.valueOf(((String) turnoComboBox.getSelectedItem()).replaceAll(" ", "_"));
						agenda.setTurno(chamada);
						RemoteAgendaTreinamentoService.getInstance().confirmTrainingSchedule(agenda, treinamento,
								chamada);
						editRegister(agenda);
						AdapitVirtualFrame.getInstance().showOperationSucess();
					} catch (Exception ex) {
						ex.printStackTrace();
						AdapitVirtualFrame.getInstance().showErrorDialog("Problemas para atualizar a agenda",
								"Nao foi possivel confirmar a agenda");
					}
			...
			cancelarButton.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					try {
						...
						ex.printStackTrace();
						AdapitVirtualFrame.getInstance().showErrorDialog("Problema ao cancelar a agenda",
								"Nao foi possivel cancelar a agenda especificada!");
					}
				}
			});
		}
	...
	private JComboBox getHoraInicioComboBox() {
		if (horaInicioComboBox == null) {
			 ...
			 * this.binder.addBindProperty(this.agenda,this.horaInicioTextField,
			 * "horaInicio");
			 * 
	...
	private JButton getNovaAgendaButton() {
		if (novaAgendaButton == null) {
			...
			novaAgendaButton.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					try {
						RemoteAgendaTreinamentoService.getInstance()
								.generateScheduleHistoryByScheduledTrainingId(treinamento.getId());
						newRegister();
					} catch (Exception ex) {
						ex.printStackTrace();
					}
	...
	private JButton getListarAgendasButton() {
		if (listarAgendasButton == null) {
			...
			listarAgendasButton.setText("Hist�rico");
	...
	private JComboBox getIntervaloComboBox() {
		if (intervaloComboBox == null) {
			...
			intervaloComboBox.addItem("Vinte Minutos, das  At� as Horas");
			intervaloComboBox.addItem("Quinze Minutos, das  At� as Horas");
			intervaloComboBox.addItem("Dez Minutos, das  At� as Horas");
			intervaloComboBox.addItem("Meia Hora, das  At� as Horas");
			/*
			 * this.binder.addBindProperty(this.agenda,this.intervaloTextField,
			 * "intervalo");
			 * 
	...
	public void editRegister(AgendaTreinamento agendaTreinamento) throws Exception {
		if (treinamento == null)
			throw new Exception("The Schedule cannot be null!!");
	...
	private void updateFields() {
		...
		if (agenda.getHoraFim() != null)
			...
			horaFimComboBox.setSelectedIndex(0);
		...
		if (agenda.getIntervalo() != null)
			...
			intervaloComboBox.setSelectedIndex(0);
		...
		try {
			int size = (Integer) RemoteAgendaTreinamentoService.getInstance()
					.getScheduledTrainingScheduleNumber(agenda.getId());
			if (size > 1) {
	...
	public void newRegister() throws Exception {
		if (treinamento == null)
			throw new Exception("The Schedule cannot be null!!");
		agenda.setId(0);
		agenda.setTurno(TurnoTreinamento.Manha);
		agenda.setConfirmada(false);
		...
		agenda.setTreinamento(treinamento);
		agenda.setObs(null);
		agenda.setStatus(StatusAgenda.Nao_cadastrada);
	...
	public void setTreinamentoCadastreForm(ScheduledTrainingCadastreForm treinamentoCadastreForm) {
		...
		try {
			if (observer.getScheduledTraining().getTreinamento() != null)
				cargaHorariaTreinamentoTextField
						.setText("" + observer.getScheduledTraining().getTreinamento().getCargaHoraria());
